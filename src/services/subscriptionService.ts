...
export const subscriptionService = {
  ...
  async deleteSubscription(id: string) {
    ...
    // 2. Delete associated commissions generated by subscription items/renewals
    ...
    // 3. Delete subscription document from 'subscriptions' and 'assinaturas' if present
    const docRef = doc(db, SUBSCRIPTIONS_COLLECTION, id);
    await deleteDoc(docRef);

    const assinaturasRef = doc(db, 'assinaturas', id);
    try {
