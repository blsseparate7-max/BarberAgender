  ...
  const handleSaveTenant = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await tenantService.updateTenant(editingTenant.id, {
        ...
        trialStartDate: isCurrentlyTrial && editTenantTrialStartDate && !isNaN(Date.parse(editTenantTrialStartDate)) ? new Date(editTenantTrialStartDate).toISOString() : (isCurrentlyTrial ? new Date().toISOString() : undefined),
        trialEndDate: isCurrentlyTrial && editTenantTrialEndDate && !isNaN(Date.parse(editTenantTrialEndDate)) ? new Date(editTenantTrialEndDate).toISOString() : (isCurrentlyTrial ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() : undefined),
