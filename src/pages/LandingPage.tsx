...
export function LandingPage({ onSelectRole, activeTenant }: LandingPageProps) {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans selection:bg-emerald-500 selection:text-zinc-950">
      ...
      <header className="relative z-10 border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-md sticky top-0" id="landing-header">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm font-medium text-zinc-400">
            ...
            <a href="#barbearias" className="hover:text-white transition-colors">Barbearias Cadastradas</a>
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 p-8 rounded-3xl relative overflow-hidden group hover:border-emerald-500/20 transition-all flex flex-col justify-between">
              ...
              <div className="pt-8">
                <a
                  href="#barbearias"
                  className="block w-full bg-indigo-600 hover:bg-indigo-500 text-white text-center py-3 px-4 rounded-xl font-bold text-sm transition-all shadow-lg shadow-indigo-600/5"
                >
      ...
      {/* Search & Tenants Grid Section */}
      <section className="relative z-10 max-w-7xl mx-auto px-6 py-20" id="barbearias">
        <div className="max-w-4xl mx-auto text-center mb-12 space-y-4">
          <h2 className="text-3xl font-extrabold tracking-tight">Barbearias na Plataforma</h2>
          <p className="text-zinc-400">Encontre a sua barbearia para fazer um agendamento ou navegue pelas unidades disponíveis.</p>
          ...
          <div className="relative max-w-md mx-auto pt-4">
            ...
            <input 
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-xl py-3 pl-12 pr-4 text-sm focus:outline-none focus:border-emerald-500/50 text-white"
            />
          </div>
        </div>
        ...
        {loadingTenants ? (
          ...
          <div className="bg-zinc-900/20 border border-zinc-900 text-center p-12 rounded-3xl max-w-md mx-auto">
            <p className="text-zinc-400 mb-4">Nenhuma barbearia encontrada com este termo.</p>
            <button 
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {filteredTenants.map((item) => (
              <div 
                key={item.id}
                className="bg-zinc-900/40 border border-zinc-800/80 hover:border-zinc-700/80 p-6 rounded-2xl flex flex-col justify-between transition-all group"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div 
                      className="w-12 h-12 rounded-xl flex items-center justify-center font-extrabold text-white text-lg shadow-inner"
                      style={{ backgroundColor: item.accentColor || '#10B981' }}
                    >
                      {item.logoUrl ? (
                        <img src={item.logoUrl} alt={item.name} className="w-full h-full object-cover rounded-xl" referrerPolicy="no-referrer" />
                      ) : (
                        item.name.substring(0, 2).toUpperCase()
                      )}
                    </div>
                    {item.id === 'barber-elite' && (
                      <span className="bg-emerald-500/10 text-emerald-400 text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase border border-emerald-500/10">
                        Principal
                      </span>
                    )}
                  </div>

                  <h4 className="text-lg font-bold text-white mb-1 group-hover:text-emerald-400 transition-colors">{item.name}</h4>
                  
                  {item.address && (
                    <div className="flex items-start gap-2 text-zinc-500 text-xs mb-3">
                ...
                <button
                  ...
                  className="w-full bg-zinc-800 hover:bg-emerald-500 hover:text-zinc-950 text-white font-bold py-2.5 rounded-xl text-xs transition-all flex items-center justify-center gap-2 mt-4"
                >
                  <Calendar size={14} /> Agendar Agorinha
                </button>
              </div>
            ))}
          </div>
